class explosion{
	constructor(x, y, t, preset){
		...
		this.nParts = preset.nParts;
		this.shotAngle = preset.shotDir
		this.preset = preset
		this.onCreate(t)
	}
	...
	onCreate(t){
		...
		var shotSpeed = random(this.preset.shotSpeed[0], this.preset.shotSpeed[1])
		var rad = random(this.preset.radius[0], this.preset.radius[1])
		// var angle = p5.Vector.fromAngle(random(0, TAU), shotSpeed)
		
		var a = random(this.preset.angleRange[0], this.preset.angleRange[1])
		var angle = p5.Vector.fromAngle(this.preset.shotDir + a, shotSpeed)

		let pColour = color(t%360, map(shotSpeed, 2, 50, 100, 0), 100)
		if (this.preset.col != undefined){
			pColour = this.preset.col
		}
...
class particleSystem{
	...
	addExplosion(x, y, preset){
		this.explosions.push(new explosion(x, y, this.time, preset))
	}
