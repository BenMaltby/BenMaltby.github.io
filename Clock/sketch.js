    ...
    for (let ps of groups){
      for (let p = 0; p < ps.length; p++){
        ...
        if (m < b.rad + strokeRad/2){  // In collision
          ...
          c.col = hitCol
          // c.col = color(0, 0, constrain(p5.Vector.mag(b.vel)/2, 10, 100))
        }
        ...
        if (d <= threshold && distB <= threshold && gId != 8){
          if (p < ps.length-1){
          // Line
        ...
        if (gId == 8 || gId == 7 || gId == 6){  // second Hand
          let pDist = p5.Vector.dist(c.target, createVector())
          c.target = createVector(
            pDist * cos(timeAngles[gId-6]),
            pDist * sin(timeAngles[gId-6])
          )
        }
      }
    }
...
function setup() { 
  ...
  let circleRaw = circleToPoints(0, 0, clockRadius, clockN)
  circleGroup = generateDotPath(circleRaw, clockN)
  ...
  secondGroup = generateDotPath(secondRaw, clockN/2)
...
function circleToPoints(x, y, r, n){
  let final = []
  for (let i = 0; i < TAU + TAU/n; i+=TAU/n){
    final.push(r * cos(i) + x)
    final.push(r * sin(i) + y)
  }
  return [final];
}

function lineToPoints(x1, y1, x2, y2, n){
  let final = []
